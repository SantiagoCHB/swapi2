...
import { doc, getDoc, setDoc, updateDoc } from 'firebase/firestore'; import { db, auth } from '../../firebase/firebaseConfig'; import { onAuthStateChanged } from 'firebase/auth'; 
...
const guardarResultado = async (acierto) => {
  ...
  try {
    ...
    // Actualizar estadísticas del usuario sin variable intermedia
...
`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokem on/other/official-artwork/${id}.png` 
 ); 
 setLoading(false); 
 } catch (err) {
...
const handleLetterClick = async (letter) => { 
 ...
 const updatedGuessed = [...guessedLetters, letter];  setGuessedLetters(updatedGuessed); 
...
return ( 
 <ScrollView contentContainerStyle={styles.container}>  <Text style={styles.title}>Adivina el Pokémon</Text> 
 ...
 <> 
 <Text>{pokemonId}</Text> 
 <Image source={{ uri: pokemonImage }} style={styles.image} />  <View style={styles.wordContainer}>{renderWord()}</View> 
...
onPress={restartGame}> 
 <Text style={styles.buttonText}>Jugar otra vez</Text>  </TouchableOpacity> 
 )} 
 </> 
 )} 
 </ScrollView> 
...
image: { width: 150, height: 150, marginVertical: 10 }, stats: { marginBottom: 10, fontSize: 16 }, 
